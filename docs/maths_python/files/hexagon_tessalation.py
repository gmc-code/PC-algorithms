...
def tes_hexagon(t, screen_width, screen_height, side_length, start_h=0):
    ...
    yrep = int(screen_height / slsqr3)
    print(yrep)
    for i in range(yrep):
        for j in range(xrep):
            if (i % 2) == 0:
                start_pos = (-screen_width//2 + (j * 3 * side_length), screen_height//2 - slsqr3 - (i * slsqr3))
            else:
                start_pos = (-screen_width//2 + (j * 3 * side_length) + (1.5 * side_length), screen_height//2 - slsqr3 - (i * slsqr3))
