...
.. code-block:: vcaapseudocode

    DEFINE procedure newton(f(x), f'(x), initial_guess, maximum_iterations, tolerance)

        i ← 0
        x0 ← initial_guess

        WHILE i < maximum_iterations DO
            x1 ← x0 - f(x0) / f'(x0)

            IF absolute_value(x1 - x0) ≤ tolerance THEN
                RETURN x1
            END IF
